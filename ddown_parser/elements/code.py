...
class InlineCodeElement(DdownElement):
    """Represents an inline code element in the Ddown document."""
    
    def __init__(self, content: str, attributes: Optional[Dict[str, Any]] = None) -> None:
        """Initialize a new inline code element.
        
        Args:
            content: Content of the inline code
            attributes: Additional attributes for the inline code
        """
        super().__init__('inline_code', content, attributes)
    
    def to_html(self) -> str:
        """Convert the inline code element to HTML.
        
        Returns:
            HTML representation of the inline code element
        """
        attributes_str = self._build_html_attributes()
        ...
        return f"<code{attributes_str}>{content}</code>"
